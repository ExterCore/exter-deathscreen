...
RegisterNetEvent('exter-deathscreen:openUI', function(time)
    isDead = true
...
RegisterNetEvent('exter-deathscreen:revive', function()
    isDead = false
...
RegisterNetEvent('exter-deathscreen:die', function(time)
    isDead = true
    SendNUIMessage({action = "DSMenu", open = true, time = time})
    if Config.Blur then
        TriggerScreenblurFadeIn()
...
RegisterNetEvent('exter-deathscreen:res', function()
    SendNUIMessage({action = "res"})
end)

RegisterNetEvent('exter-deathscreen:updateRes', function(time)
    SendNUIMessage({action = "updateRes", time = time})
end)

RegisterCommand('test',function()
    TriggerEvent('exter-deathscreen:updateRes', 0)
end)
